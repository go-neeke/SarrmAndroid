...
class ReplySettingListAdapter (val clickListener: ClickListener) : ListAdapter<ReplySetting, ReplySettingListAdapter.ViewHolder>(PlantDiffCallback()) {
...
class ClickListener(val clickListener: (replySettingId: String)-> Unit) {
    fun onClick(replySetting: ReplySetting) = clickListener(replySetting.id)
}